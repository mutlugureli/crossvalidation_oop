...
class Foods():
    ...
    def laoddataset(self):
        self.food = pd.read_csv(self.pathway)

    def dropnulls(self):
        self.food = self.food.dropna(axis=1)
        self.food = self.food.drop(["ID", "ShortDescrip", "Descrip"], axis = 1)

    def labelencoding(self):
        label_quality = LabelEncoder()
        self.food[self.column_to_predict] = label_quality.fit_transform(self.food[self.column_to_predict])

    def traindatapreprocess(self):
        X = self.food.drop(self.column_to_predict, axis=1) #quality sütununu ayırdık ve geri kalanlar x tablosu oldu.
        y = self.food[self.column_to_predict]
...
class RandomForestClassifying(Foods):
    def __init__(self, pathway, column_to_predict, n_estimators = 200):
...
class SupportVectorMachineClassifying(Foods):
    def __init__(self, pathway, column_to_predict):
        super().__init__(pathway, column_to_predict)
...
class MultilayerPerceptronClassifying(Foods):
    def __init__(self, pathway, column_to_predict, hidden_layer_sizes=(11,11,11), max_iter=1000):
        super().__init__(pathway, column_to_predict)
...
class Reports(RandomForestClassifying, SupportVectorMachineClassifying, MultilayerPerceptronClassifying):
    def __init__(self, pathway, column_to_predict):
        ...
        MultilayerPerceptronClassifying.__init__(self, pathway, column_to_predict, hidden_layer_sizes=(11,11,11), max_iter=500)
        self.reportPrint()
...
class CrossValidation(RandomForestClassifying, SupportVectorMachineClassifying, MultilayerPerceptronClassifying):
    def __init__(self, pathway, column_to_predict):
        ...
        SupportVectorMachineClassifying.__init__(self, pathway, column_to_predict)
        MultilayerPerceptronClassifying.__init__(self, pathway, column_to_predict, hidden_layer_sizes=(3,3,3), max_iter=500)
...
# a = RandomForestClassifying(r"C:\Users\Monster PC\Desktop\Datasets\nndb_flat.csv", "FoodGroup")
# b = SupportVectorMachineClassifying(r"C:\Users\Monster PC\Desktop\Datasets\nndb_flat.csv", "FoodGroup")
# c = MultilayerPerceptronClassifying(r"C:\Users\Monster PC\Desktop\Datasets\nndb_flat.csv", "FoodGroup")
# d = Reports(r"C:\Users\Monster PC\Desktop\Datasets\nndb_flat.csv", "FoodGroup")
e = CrossValidation(r"C:\Users\Monster PC\Desktop\Datasets\nndb_flat.csv", "FoodGroup")
